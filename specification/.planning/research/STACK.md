...
**OXC feature flags (on the umbrella crate):**

```toml
oxc = { version = "0.123", features = [
    "codegen",    # oxc_codegen: AST -> JavaScript source output
    "semantic",   # oxc_semantic: SemanticBuilder for scope/symbol/reference analysis
    "serialize",  # ESTree JSON serialization (testing/debugging)
    "ast_visit",  # VisitMut trait for pre-pass mutations (const_replace)
] }
```

**Feature flags explicitly excluded:**
...
**Why MEDIUM confidence on NAPI v3:** The SWC Qwik optimizer uses NAPI v2 + separate wasm-bindgen for WASM. NAPI v3 promises unified Node+WASM from one codebase, which would eliminate the separate WASM crate entirely. OXC itself uses this pattern. However, the Qwik optimizer's WASM target has specific constraints (browser execution without WASI runtime) that need validation. If NAPI v3 WASM doesn't work for browser-only contexts, fall back to the proven wasm-bindgen pattern.
...
**OXC releases 2-3 times per week** (0.113 -> 0.123 in ~6 weeks = ~20 releases). This is pre-1.0 software with frequent breaking changes.

**Recommendation:** Pin to minor version (`0.123`) not exact patch. Update OXC versions in batches during dedicated upgrade passes, not continuously. Run the 162-spec test suite after each upgrade to catch regressions.

Jack's code pins to `0.113` (approximately 6 weeks behind latest). This is fine during active development. Before shipping v1, upgrade to latest and lock.

**Critical constraint:** `oxc` and `oxc_traverse` MUST be the same version. They share internal types (arena allocators, AST nodes) that are not ABI-stable across versions.
...
qwik-optimizer-next/
  ...
  crates/
    qwik-core/            # Core optimizer logic (parser, transforms, codegen)
      Cargo.toml          # oxc, oxc_traverse, serde, anyhow, base64, etc.
    qwik-napi/            # Node.js bindings
      Cargo.toml          # napi v3, napi-derive v3, qwik-core (path dep)
    qwik-wasm/            # WASM bindings (if needed)
      Cargo.toml          # wasm-bindgen OR napi wasm target
  specification/          # This project (spec + tests)
```

The core crate MUST NOT depend on napi or wasm-bindgen. Binding crates are thin wrappers that call into core. This is the same pattern used by both the SWC Qwik optimizer and OXC itself.
...
cargo init --lib crates/qwik-core
cargo init --lib crates/qwik-napi
...
cd ../qwik-napi
cargo add napi@3 --features serde-json
cargo add napi-derive@3
```
